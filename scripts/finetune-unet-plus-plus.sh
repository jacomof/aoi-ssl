...
#SBATCH --mem=30gb
#SBATCH --output=logs/output/unet_plus_output_%x-%J.out
#SBATCH --error=logs/error/unet_plus_output_%x-%J.err
...
python -m segmentation.train --config ./configs/finetune/finetune_main.yml \
    --model_config ./configs/finetune/finetune_unet_plus_plus.yml \
    --model unet_plus_plus \
    --run_key "finetune_unet_plus_plus" \
    --path "./checkpoints/" \
