...
class DataAugmentationiBot(object):
    ...
    def __init__(self, global_crops_scale, local_crops_scale, local_crops_number, patch_size, use_swin=False, mim_start_epoch=0, base_augmentations=False, normalize=False):

        if base_augmentations:
            print("Using base augmentations for DINO pretraining.")
        ...
        if use_swin:
            print("Using Swin Transformer, patch size will be multiplied by 8.")
            self.patch_size = patch_size * 8 # 3 downsamples of stride 2 -> (2**3)
            print(f"Patch size for Swin Transformer: {self.patch_size}")

        print(f"MIM start epoch: {self.mim_start_epoch}")

        flip_and_color_jitter = A.Compose([
            A.HorizontalFlip(p=0.5),
            ColorJitterFor2Channel(brightness=0.4, contrast=0.4, saturation=0.2, hue=0.1, p=0.8)
        ])

        old_normalize = A.Compose([
            A.Normalize((0.485, 0.456), (0.229, 0.224)), # A.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)),
            ToTensorV2()
        ])
        ...
        augmentations1 = old_augmentations1 if base_augmentations else new_augmentations1
        augmentations2 = old_augmentations2 if base_augmentations else new_augmentations2

        if normalize:
            print("Normalization is enabled. Adding normalization transform.")
    ...
    def __call__(self, image, mask, current_epoch):
        ...
        for i in range(2):
            ...
            if should_mask and current_epoch >= self.mim_start_epoch:  # Start masking after epoch 50
                prediction_ratio = random.uniform(0.1, 0.5)
                masks.append(self.input_mask_generator(crops[i], patch_size=self.patch_size, prediction_ratio=prediction_ratio))
            else:
                prediction_ratio = 0.0
                masks.append(self.input_mask_generator(crops[i], patch_size=self.patch_size, prediction_ratio=prediction_ratio))

        for i in range(2, len(crops)):
            masks.append(self.input_mask_generator(crops[i], patch_size=self.patch_size, prediction_ratio=0))
