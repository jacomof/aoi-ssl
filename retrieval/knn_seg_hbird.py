...
class SafeJaccardIndex:
    ...
    def __call__(self, preds, target):
        ...
        for i, key in enumerate(self.jaccard_metric.labels):
            ...
            predict_positive_count = preds[:, i, :, :].sum()
...
class KNNHummingBirdSegmentation():
    ...
    def __init__(self, 
        ...
        self.num_batches = num_batches  # batch size = 32, 31 batches, 10 features per image
        ...
        if config is not None:
            ...
            print(f"Using image lists for training and validation.")
        ...
        self.num_sampled_features = 1024 if num_sampled_features is None else num_sampled_features  # Fixed number of features to sample per image
        self.total_batches = len(self.train_loader)
        self.max_batches = config.max_batches if hasattr(config, 'max_batches') else 1000
        self.num_batches = min(self.num_batches, self.max_batches)  # Limit the number of batches to process
        self.memory_size = self.batch_size * self.num_batches * self.num_sampled_features  # batch size = 32, 31 batches, 10 features per image
        if self.num_batches == -1:
            self.memory_size = None  # Unbounded memory size
        dataset_size = len(self.train_loader.dataset) if self.train_loader is not None else 0
        ...
        if self.memory_size is not None:
            ...
            self.feature_memory = torch.zeros((self.memory_size, self.encoder.embed_dim))
            print(f"Creating memory of size {self.memory_size} with {self.num_sampled_features} features per image.")
            print(f"Creating label memory of size {self.memory_size} with {self.num_classes} classes.")
    ...
    def create_NN(self, n_neighbours=30, nn_method='faiss', feature_memory = None, **kwargs):

        feature_memory = self.feature_memory if feature_memory is None else feature_memory
        if self.nn_method == 'faiss':
            self.NN_algorithm = NearestNeighborSearchFaiss(self.feature_memory, n_neighbors=n_neighbours, **kwargs)
        else:
            raise ValueError(f"Unsupported nearest neighbor method: {self.nn_method}. Supported methods: 'faiss'.")

    def find_nearest_key_to_query(self, q:torch.Tensor, k:Optional[int] = None, 
                                  feature_memory:torch.Tensor = None, label_memory:torch.Tensor = None, return_distance:bool = False):
        ...
        num_pixels = self.patch_size * self.patch_size  # Assuming patch_size is defined in the class
        # distance shape -> (bs*num_patches, k)
        print(f"Finding nearest neighbors for {bs*num_patches} queries with k={k} using {self.nn_method} method.")
        ...
        key_features = key_features.reshape(bs, num_patches, k, -1) # shape (bs, num_patches, k, d_k)
        key_labels = self.label_memory[neighbors] # shape (bs*k, num_pixels, num_classes)
        key_labels = key_labels.reshape(bs, num_patches, k, num_pixels, -1) # shape (bs, num_patches, k, patch_size*patch_size, num_classes)
    ...
    def get_patch_scores(self, gt, class_frequency):
        ...
        for i in range(gt.shape[0]):
            for j in range(gt.shape[1]):
                ...
                patch_scores[i, j] = class_frequency[patch_classes].sum() # sum of class frequencies in the patch
                nonzero_indices[i, j] = patch_classes.shape[0] > 0 # if there are any classes in the patch
    ...
    def patchify_gt(self, gt, patch_size):
        ...
        gt = gt.reshape(bs, h//patch_size, w//patch_size, patch_size*patch_size, c)
    ...
    def cross_attention(self, q, k, v, beta=0.02):
        ...
        """
        print(f"Cross-attention with beta: {beta}, q shape: {q.shape}, k shape: {k.shape}, v shape: {v.shape}")
        d_k = q.size(-1)
    ...
    def cross_attention_simple_multi_beta(self, q, k, v, beta_per_class=None):
        ...
        """
        print(f"Cross-attention with beta: {beta_per_class}, q shape: {q.shape}, k shape: {k.shape}, v shape: {v.shape}")
        ...
        base_scores = torch.einsum("bpd,bpnd->bpn", q_norm, k_norm)  # (bs, num_patches, NN)
        
        class_predictions = []
        
        for class_idx, beta_class in enumerate(beta_per_class):
            # Apply class-specific temperature
            class_attn = F.softmax(base_scores / beta_class, dim=-1)  # (bs, num_patches, NN)
            
            # Get class-specific values
            class_values = v[:, :, :, class_idx]  # (bs, num_patches, NN)
            
            # Weighted sum for this class
            class_pred = torch.sum(class_attn * class_values, dim=-1, keepdim=True)  # (bs, num_patches, 1)
            class_predictions.append(class_pred)
        
        # Stack all class predictions
        label_hat = torch.cat(class_predictions, dim=-1)  # (bs, num_patches, num_classes)
    ...
    def cross_attention_complete_patches(self, q, k, v, beta=0.02):
        ...
        attn_scores = torch.einsum("bnld,bnmd->bnlm", q_expanded, k) / beta  # (bs, num_patches, 1, NN)
        attn_weights = F.softmax(attn_scores.squeeze(2), dim=-1)  # (bs, num_patches, NN)
        
        # Apply attention to complete patch labels
        attn_weights_expanded = attn_weights.unsqueeze(-1).unsqueeze(-1)  # (bs, num_patches, NN, 1, 1)

        # shape v: (bs, num_patches, NN, patch_size*patch_size, num_classes)
        # Weighted combination of complete patch labels
        reconstructed_patches = torch.sum(attn_weights_expanded * v, dim=2)  # (bs, num_patches, patch_size*patch_size, num_classes)
    ...
    def sample_features_vectorized(self, features, patchified_gts):
        ...
        class_frequency = self.get_class_frequency_vectorized(patchified_gts)  # Shape: (bs, num_classes)
        
        # Vectorized patch scores computation
        patch_scores, nonzero_indices = self.get_patch_scores_vectorized(patchified_gts, class_frequency)
        ...
        _, indices = torch.topk(patch_scores, self.num_sampled_features, largest=False, dim=1)
        
        # Vectorized feature sampling using advanced indexing
        batch_indices = torch.arange(bs).unsqueeze(1).expand(-1, self.num_sampled_features)
        sampled_features = features[batch_indices, indices]  # Shape: (bs, num_sampled_features, d_k)
    ...
    def get_class_frequency_vectorized(self, patchified_gts):
        ...
        presence_indicators = patchified_gts.sum(dim=3) > 0  # Shape: (bs, spatial_res_h, spatial_res_w, num_classes)

        class_frequency = presence_indicators.sum(dim=(1,2)) # Shape: (bs, num_classes)
    ...
    def get_patch_scores_vectorized(self, patchified_gts, class_frequency):
        ...
        gt_reshaped = presence_indicators.view(bs, spatial_res_h * spatial_res_w, num_classes)  # Shape: (bs, num_patches, num_classes)
        ...
        patch_scores = torch.sum(gt_reshaped * class_freq_expanded, dim=2).float()  # Shape: (bs, num_patches)
    ...
    def get_class_frequency(self, gt):
        ...
        for i in range(gt.shape[0]):
            ...
            for j in range(gt.shape[1]):
                ...
                patch_classes = gt[i, j].unique() # preconfigured for multi-label segmentation
    ...
    def create_memory(self, train_loader, config, average_patches=True):
        ...
        with torch.no_grad():
            for j in range(self.augmentation_epoch):
                ...
                for i, data in enumerate(train_loader):
                    if self.profile_time:
                        ...
                        print(f"Time taken to access data: {time_end_access - time_start_access:.2f} seconds")
                        time_start_access = time_end_access
                    if self.profile_memory:
                        print(f"Memory usage cuda: {torch.cuda.memory_allocated() / (1024 ** 2):.2f} MB")
                    ...
                    features = features["x_norm_patchtokens"]  # Shape: (bs, num_patches, d_k)
                    normalized_features = features / torch.norm(features, dim=2, keepdim=True)
                    assert torch.allclose(torch.norm(normalized_features, dim=2), torch.tensor(1.0)), "Features should be normalized"

                    patch_size = self.encoder.patch_embed.patch_size[0]
                    patchified_gts = self.patchify_gt(y, patch_size) ## (bs, spatial_resolution, spatial_resolution, patch_size*patch_size, num_classes)
                    ...
                    if self.memory_size is None:
                        ...
                        normalized_features = normalized_features.flatten(0, 1) # shape: (bs * num_patches, d_k)
                        label = label.flatten(0, 2) # flatten the label map to match the features
                        ...
                        sampled_features, sampled_indices = self.sample_features_vectorized(features, patchified_gts)
                        ...
                        label_hat = label.gather(1, sampled_indices.unsqueeze(-1).expand(-1, -1, label.shape[-1]))
            ...
            print(f"Finished creating memory with {self.feature_memory.shape[0]} features and {self.label_memory.shape[0]} labels.")
    ...
    def multi_scale_cross_attention(self, q, k, v, beta=0.02):
        ...
        v_patch_avg = v.view(*v.shape[:-1], patch_size * patch_size, self.num_classes).mean(dim=-2)  # Average within patches
        patch_predictions = self.cross_attention(q, k, v_patch_avg, beta)  # (bs, num_patches, num_classes)
        
        # 2. Pixel-level attention within patches
        v_reshaped = v.view(*v.shape[:-1], patch_size * patch_size, self.num_classes)  # (bs, num_patches, NN, pixels_per_patch, num_classes)
        
        # Compute attention weights (same as before)
        q_norm = F.normalize(q, dim=-1)
        k_norm = F.normalize(k, dim=-1)
        attn_weights = F.softmax(
            torch.einsum("bpd,bpnd->bpn", q_norm, k_norm) / beta, 
            dim=-1
        )  # (bs, num_patches, NN)
        ...
        pixel_predictions = torch.sum(
            attn_weights_expanded * v_reshaped, 
            dim=2
        )  # (bs, num_patches, pixels_per_patch, num_classes)
        
        # 3. Combine patch and pixel predictions
        patch_pred_expanded = patch_predictions.unsqueeze(-2).expand(-1, -1, patch_size * patch_size, -1)  # (bs, num_patches, pixels_per_patch, num_classes)
        
        # Weighted combination
        alpha = 0.3  # Weight for patch-level predictions
        combined_predictions = alpha * patch_pred_expanded + (1 - alpha) * pixel_predictions
    
        return combined_predictions.view(q.shape[0], q.shape[1], -1)  # Flatten back to (bs, num_patches, patch_size*patch_size*num_classes)
    ...
    def reshape_patches_to_spatial(self, predictions, original_h, original_w):
        ...
        predictions = predictions.view(
            B, 
            spatial_resolution, spatial_resolution,  # patch grid
            self.patch_size, self.patch_size,        # pixels within each patch
            num_classes
        )  # (B, spatial_res, spatial_res, patch_size, patch_size, num_classes)
        
        # Step 2: Rearrange to reconstruct full spatial image
        predictions = predictions.permute(0, 5, 1, 3, 2, 4)  # (B, num_classes, spatial_res, patch_size, spatial_res, patch_size)
        ...
        if patch_h != original_h or patch_w != original_w:
            print(f"Interpolating predictions to original size: ({original_h}, {original_w})")
    ...
    def predict_average(self, x_test: torch.Tensor, k: Optional[int] = None,  feature_memory: Optional[torch.Tensor] = None,
                label_memory: Optional[torch.Tensor] = None, beta: Optional[int] = None) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            features = self.encoder(x_test)["x_norm_patchtokens"]  # Shape: (batch_size, num_patches, d_k)
            ...
            key_features, key_labels = self.find_nearest_key_to_query(q, feature_memory=feature_memory, label_memory=label_memory, k=k)           
            label_hat =  self.cross_attention(features_normalized, key_features, key_labels, beta)
            bs, _, label_dim = label_hat.shape
            combined_labels = label_hat.reshape(bs, eval_spatial_resolution, eval_spatial_resolution, label_dim).permute(0, 3, 1, 2)
            resized_label_hats =  F.interpolate(combined_labels.float(), size=(h, w), mode="bilinear")

        return resized_label_hats
    
    def predict_multi_betas(self, x_test: torch.Tensor, k: Optional[int] = None, 
                            feature_memory: Optional[torch.Tensor] = None,
                            label_memory: Optional[torch.Tensor] = None,
                            beta_per_class: Optional[List[float]] = None) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            q = features_normalized.clone().detach()
            key_features, key_labels = self.find_nearest_key_to_query(q, feature_memory=feature_memory, label_memory=label_memory, k=k)           
            label_hat =  self.cross_attention_simple_multi_beta(features_normalized, key_features, key_labels, beta_per_class)
            bs, _, label_dim = label_hat.shape
            combined_labels = label_hat.reshape(bs, eval_spatial_resolution, eval_spatial_resolution, label_dim).permute(0, 3, 1, 2)
            resized_label_hats =  F.interpolate(combined_labels.float(), size=(h, w), mode="bilinear")
    ...
    def predict_complete_patches(self, x_test: torch.Tensor, k: Optional[int] = None, 
                            feature_memory: Optional[torch.Tensor] = None,
                            label_memory: Optional[torch.Tensor] = None, 
        ...
        # DEBUG: Print prediction and input shapes and resolution
        # patch_size = self.encoder.patch_size
        # eval_spatial_resolution = self.input_resolution[0] // patch_size
        # print(f"patch_size: {patch_size}")
        # print(f"input_resolution: {self.input_resolution}")
        # print(f"eval_spatial_resolution: {eval_spatial_resolution}")
        ...
        with torch.no_grad():
            ...
            # Get features
            features = self.encoder(x_test)["x_norm_patchtokens"]  # (bs, num_patches, d_k)
    ...
    def _combine_labels_distance(self, label_masks, threshold=0.5, distances=None):
        ...
        if distances is not None:
            ...
            weights_expanded = weights.unsqueeze(-1).unsqueeze(-1)  # (B, num_patches, K, 1, 1)

            # Weighted combination
            weighted_labels = label_masks * weights_expanded
            mean_labels = weighted_labels.sum(dim=2)  # (B, num_patches, num_pixels, num_classes)
        else:
    ...
    def predict_distance(self, x_test: torch.Tensor, k: Optional[int] = None,
                         feature_memory: Optional[torch.Tensor] = None,
                         label_memory: Optional[torch.Tensor] = None,) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            q = features_normalized.clone().detach()
            print(f"k is: {k}")
            key_features, key_labels, similarities = self.find_nearest_key_to_query(q, feature_memory=feature_memory, label_memory=label_memory, k=k, return_distance=True)

            print(f"Average similarities: {similarities.mean()}")  # Debugging line to check distances

            label_hat =  self._combine_labels_distance(key_labels, distances=(1-similarities))  # (bs, num_patches, num_classes)
            bs, num_patches, num_pixels, label_dim = label_hat.shape
            predictions = self.reshape_patches_to_spatial(label_hat, h, w)  # (bs, num_classes, h, w)
    ...
    def evaluate_distance(self, k=10, threshold=0.5) -> dict:
        ...
        with torch.no_grad():
            for batch in self.val_loader:
                ...
                class_mask = batch["class_mask"].permute(0, 3, 1, 2).long()  # Change shape to (B, C, H, W)

                
                x_test = batch["image"]
                combined_labels = self.predict_distance(x_test, k, 
                                                        feature_memory=self.feature_memory, 
                                                        label_memory=self.label_memory) # Shape: (batch_size, H, W, C)

                combined_labels_thresholded = (combined_labels > threshold).float() # Thresholding the predictions
                
                
                start_time = time.time()

                classwise_ious.append(self.iou_classwise(combined_labels_thresholded, class_mask))
    ...
    def evaluate(self, patch_mechanism="complete", attention_mechanism="normal", k=None, thresholds=None, betas=None):
        """
        ...
        with torch.no_grad():
            for batch in self.val_loader:
                ...
                if patch_mechanism == "average":
                    if type(betas) == list:
                        resized_label_hats = self.predict_multi_betas(x_test, k=k, beta_per_class=betas, 
                                                  feature_memory=feature_memory, label_memory=label_memory).permute(0, 2, 3, 1) # Shape: (batch_size, H, W, C)
                    else:
                        resized_label_hats = self.predict_average(x_test, k=k, beta=betas, feature_memory=feature_memory, 
                                                  label_memory=label_memory).permute(0, 2, 3, 1) # Shape: (batch_size, H, W, C)
                elif patch_mechanism == "complete":
                    resized_label_hats = self.predict_complete_patches(x_test, k=k, beta=betas, 
                                                  feature_memory=feature_memory, label_memory=label_memory,
                                                  attention_mechanism=attention_mechanism)
                    print(f"Resized label hats shape: {resized_label_hats.shape}")
                    resized_label_hats = resized_label_hats.permute(0, 2, 3, 1) # Shape: (batch_size, H, W, C)
    ...
    def test(self, config, patch_mechanism="average", attention_mechanism="normal", k=None, thresholds=None, beta=None):
        """
        ...
        with torch.no_grad():
            for batch in self.test_loader:
                ...
                if patch_mechanism == "average":
                    if type(beta) == list:
                        resized_label_hats = self.predict_multi_betas(x_test, k=k, beta_per_class=beta, 
                                                  feature_memory=feature_memory, label_memory=label_memory).permute(0, 2, 3, 1) # Shape: (batch_size, H, W, C)
                    else:
                        resized_label_hats = self.predict_average(x_test, k=k, beta=beta, feature_memory=feature_memory, 
                                                  label_memory=label_memory).permute(0, 2, 3, 1) # Shape: (batch_size, H, W, C)
                elif patch_mechanism == "complete":
                    resized_label_hats = self.predict_complete_patches(x_test, k=k, beta=beta, 
                                                  feature_memory=feature_memory, label_memory=label_memory,
                                                  attention_mechanism=attention_mechanism).permute(0, 2, 3, 1) # Shape: (batch_size, H, W, C)
