...
class PredsmIoU:
    ...
    def compute(self, is_global_zero: bool, many_to_one: bool = False,
                precision_based: bool = False, linear_probe : bool = False, 
                thresholds: list = None) -> Tuple[float, List[np.int64],
    ...
    def compute_miou(self, gt: np.ndarray, pred: np.ndarray, num_pred: int, num_gt:int,
        ...
        for i_part in range(0, num_gt):
            ...
            tmp_pred = (reordered_preds[..., i_part] > self.threshold[i_part]).flatten()  # Pred binary mask for class i_part
            if tmp_all_gt.sum() == 0 and tmp_pred.sum() == 0:
                # If both GT and prediction are empty, don't count this class
                print(f"Class {self.class_names[i_part]} has no positive examples in GT or predictions, skipping.")
                jac[i_part] = None
            else:
    ...
    def compute_binary_iou(self, gt_binary: np.ndarray, pred_binary: np.ndarray) -> float:
        """
    ...
    def _hungarian_match(self, num_pred: int, num_gt: int, pred: np.ndarray, gt: np.ndarray) -> Tuple[np.ndarray,
                                                                                                      np.ndarray]:
        score_matrix = np.zeros((self.num_gt_classes, self.num_pred_classes), dtype=np.float32)
        ...
        for gt_class in range(self.num_gt_classes):
            for pred_class in range(self.num_pred_classes):
                ...
                pred_binary = (pred[:,:,:,pred_class] > self.threshold[pred_class]) # Binary mask for this pred class
