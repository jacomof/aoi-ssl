...
class PatchEvaluationCallback(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module: LitMAE):
        ...
        mean_ious = knn_model.evaluate(
            k=30, thresholds=[0.3] * len(self.classes), betas=0.02
        )
...
def run_segmentation(
    ...
    use_swin_masking = (
        config.model_params.get("use_swin", False)
        or config.model_params.get("use_fastervit_0", False)
        or config.model_params.get("use_fastervit_1", False)
    )
    ...
    if config.model_params.get("accumulate_grad_batches"):
        warnings.warn(
            f"Accumulating gradients over {config.model_params['accumulate_grad_batches']} batches."
        )
