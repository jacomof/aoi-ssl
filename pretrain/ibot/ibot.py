...
from segmentation.models.faster_vit.faster_vit_any_res import (
    faster_vit_1_ssl_any_res,
    faster_vit_0_ssl_any_res,
)
...
class iBot(MaskedAutoencoder):
    ...
    def forward_ibot(self, x, encoder, head, masks=None, compute_locals=True):
        ...
        global_inp = torch.cat(
            x[start_idx : idx_crops[0]]
        )  # Concatenate the remaining crops
        global_masks = (
            torch.cat(masks[start_idx : idx_crops[0]]) if masks is not None else None
        )
        global_embs = encoder(global_inp, global_masks)  # (2*B)xD
        cls_embs.append(global_embs["x_norm_clstoken"])  # (2*B)xD
        global_patch_embs = global_embs["x_norm_patchtokens"].chunk(
            2, dim=0
        )  # [(B, P, D), (B, P, D)]
        ...
        patch_embs_view_1_tokenized = patch_embs_view_1_after_head.reshape(
            batch_size, -1, hidden_dim
        )  # (B, P, D)

        patch_embs_view_2 = global_patch_embs[1].flatten(0, 1)  # (B*P)xD
        patch_embs_view_2_after_head = head(patch_embs_view_2)  # (B*P)xD
        patch_embs_view_2_tokenized = patch_embs_view_2_after_head.reshape(
            batch_size, -1, self.dino_out_dim
        )  # (B, P, D)

        cls_outputs_after_head: torch.Tensor = head(cls_embs)

        return (
            cls_outputs_after_head,
            patch_embs_view_1_tokenized,
            patch_embs_view_2_tokenized,
            cls_embs,
        )
    ...
    def forward(self, x: list[torch.Tensor], masks: list[torch.Tensor]):
        ...
        (
            student_cls_after_head,
            student_patch_embs_view_1_tokenized,
            student_patch_embs_view_2_tokenized,
            student_cls_before_head,
        ) = self.forward_ibot(
            x, self.encoder, self.student_dino_head, masks, compute_locals=True
        )
        student_cls_list = student_cls_after_head.chunk(self.ncrops)  # nlocal_crops + 2
        student_cls_before_head_mean = (
            student_cls_before_head.detach().mean()
        )  # [BxD, BxD] -> [BxD]
        student_embs_avg_for_log = (
            (student_cls_list[0] + student_cls_list[1]) / 2.0
        ).detach()
        student_embs_avg_for_log = F.softmax(
            student_embs_avg_for_log / self.student_temp, dim=-1
        )

        (
            teacher_cls,
            teacher_patch_embs_view_1_tokenized,
            teacher_patch_embs_view_2_tokenized,
            _,
        ) = self.forward_ibot(
            x_globals,
            ...
            self.teacher_dino_head,
            masks=None,
            compute_locals=False,
        )

        teacher_cls_centered_sharpened = self.loss.softmax_center_teacher_cls(
            teacher_cls
        )
        teacher_cls_list = teacher_cls_centered_sharpened.detach().chunk(2)
        teacher_embs = (
            teacher_cls_list[0] + teacher_cls_list[1]
        ) / 2.0  # [BxD, BxD] -> [BxD]
        ...
        teacher_patch_output = torch.cat(
            [teacher_patch_embs_view_1_tokenized, teacher_patch_embs_view_2_tokenized],
            dim=0,
        )
        teacher_patch_centered_sharpened = (
            self.loss.softmax_center_teacher_patch(teacher_patch_output)
            .detach()
            .chunk(2)
        )
