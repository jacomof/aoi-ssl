...
class DinoMaskedAutoencoder(MaskedAutoencoder):
    ...
    def __init__(self, **parameters):
        ...
        print("Student temperature: ", self.student_temp)
        print("Teacher temperature: ", self.teacher_temp)
    ...
    def forward(self, x, mask_ratio):
        ...
        student_cls_after_head, embs_student = self.forward_dino(x, self.encoder, self.student_dino_head, self.embs_before_head)
        student_cls_list = student_cls_after_head.chunk(self.ncrops) # nlocal_crops + 2
        
        teacher_cls, embs_teacher = self.forward_dino(x_globals, self.encoder_teacher, self.teacher_dino_head, self.embs_before_head)

        #embs = teacher_cls[0]
        
        # [2*B, D] -> [2*B, D]
        if self.centering_mode == "vanilla":
            #print("Vanilla softmax applied to teacher cls")
            teacher_cls_softmaxed = self.dino_loss.softmax_vanilla_teacher(teacher_cls)
            # teacher_cls_centered -> teacher_cls_list: [2*B, D] -> [BxD , BxD]
            teacher_cls_list = teacher_cls_softmaxed.detach().chunk(2)
        elif self.centering_mode == "sharpen":
            #print("Sharpening only applied to teacher cls")
            teacher_cls_sharpened = self.dino_loss.softmax_sharpen_only_teacher(teacher_cls)
            # teacher_cls_softmaxed -> teacher_cls_list: [2*B, D] -> [BxD , BxD]
            teacher_cls_list = teacher_cls_sharpened.detach().chunk(2)
        elif self.centering_mode == "center":
            #print("Centering only applied to teacher cls")
            teacher_cls_centered = self.dino_loss.softmax_center_only_teacher(teacher_cls)
            # teacher_cls_centered -> teacher_cls_list: [2*B, D] -> [BxD , BxD]
            teacher_cls_list = teacher_cls_centered.detach().chunk(2)
        else:
            #print("Centering and sharpening applied to teacher cls")
            teacher_cls_centered_sharpened = self.dino_loss.softmax_center_teacher(teacher_cls)
        ...
        if self.normalize_loss:
            self.update_running_min_max(dino_loss, self.dino_loss_running_min, self.dino_loss_running_max)
            if self.dino_loss_running_max <= self.dino_loss_running_min:
                dino_loss = torch.tensor(0.0)
            else:
                dino_loss = (dino_loss - self.dino_loss_running_min) / (self.dino_loss_running_max - self.dino_loss_running_min)
        ...
        if self.use_mae:
            ...
            if self.normalize_loss:
                self.update_running_min_max(mae_loss, self.mae_loss_running_min, self.mae_loss_running_max)
                if self.mae_loss_running_max <= self.mae_loss_running_min:
                    mae_loss = torch.tensor(0.0)
                else:
                    mae_loss = (mae_loss - self.mae_loss_running_min) / (self.mae_loss_running_max - self.mae_loss_running_min)
