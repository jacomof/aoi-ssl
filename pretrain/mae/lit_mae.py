...
class LitMAE(PLBaseModel):
    def __init__(
        self,
        parameters: dict,
        model: MaskedAutoencoder|DinoMaskedAutoencoder|MaskedAutoencoderFasterVit = MaskedAutoencoder,
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        self.log("train/loss", loss, sync_dist=True, batch_size=batch_size, on_step=True, on_epoch=True)
    ...
    def on_train_start(self):
        ...
        if self.custom_loading:
            ...
            if self.ckpt_path is not None:
                ...
                if "optimizer_states" in checkpoint and len(self.trainer.optimizers) > 0:
                    optimizer_state = checkpoint["optimizer_states"][0]
                    self.trainer.optimizers[0].load_state_dict(optimizer_state)
                    print(f"Loaded optimizer state from checkpoint: {self.ckpt_path}")

            steps_per_epoch = len(self.trainer.datamodule.train_dataloader())

            self.trainer.current_epoch = self.custom_current_epoch
            self.trainer.global_step = self.custom_current_epoch * steps_per_epoch  # If you know steps_per_epoch
