...
class MaskedAutoencoder(nn.Module):
    def __init__(self, **parameters):
        ...
        self.num_channels = parameters["in_chans"]
        self.num_reg_tokens = parameters["reg_tokens"]
    ...
    def unpatchify(self, x):
        ...
        x = x.reshape(shape=(x.shape[0], h, w, p, p, self.num_channels))
        x = torch.einsum("nhwpqc->nchpwq", x)
        imgs = x.reshape(shape=(x.shape[0], self.num_channels, h * p, h * p))
        return imgs
...
class MaskedDecoder(nn.Module):
    def __init__(
        ...
        self.embed_dim = vit_embed[vit_type]
