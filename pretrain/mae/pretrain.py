...
class KNNEvaluationCallback(pl.Callback):
    def __init__(self, batch_size, eval_data_path, classes, num_workers, input_resolution, eval_frequency, random_seed=42):
    ...
    def on_train_epoch_end(self, trainer, pl_module:LitMAE):
        ...
        print(f"Mean IoU for epoch {trainer.current_epoch}: {mean_iou:.4f}")
...
def run_segmentation(
    ...
    print(f"Validation will be checked every {check_val_every_n_epoch} epochs.")
    print(f"Logging every {config.log_every_n} steps.")
    ...
    trainer = pl.Trainer(
        ...
        sync_batchnorm=True,
        accumulate_grad_batches=config.accumulate_grad_batches if hasattr(config, 'accumulate_grad_batches') else 1,
        accelerator="gpu"
    )
...
if __name__ == "__main__":
    ...
    if hasattr(config, "mask_ratio"):
        config.model_params["mask_ratio"] = config.mask_ratio if config.mask_ratio else 0.75
        print("Mask ratio set to: ", config.model_params["mask_ratio"])
    if hasattr(config, "lr"):
        config.model_params["lr"] = config.lr if config.lr else 1e-4    

    print("Training data path: ", config.data_path)
    print(f"Full config:\n{config}")
