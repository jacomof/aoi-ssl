...
class Metrics:
    ...
    def __init__(self, embeddings: torch.Tensor, approx: bool = True):
        ...
        self.svs, self.num_svs = self._compute_svd(
            embeddings, approx=approx, normalize=True
        )
    ...
    def _compute_svd(
        self, embs: torch.Tensor, approx: bool = True, normalize: bool = False
    ):
    ...
    def auc_embedding_collapse(
        ...
        auc = auc_norm.item()
        assert (
            auc >= 0.49 and auc <= 1.1
        ), f"AUC value is {auc}, which is not in the range [0.5, 1]"
        return auc
