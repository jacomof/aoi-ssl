...
class StaticPool(nn.Module):
    def __init__(self, output_size=(8, 8)):
        ...
        )  # Example static pooling

    def forward(self, x):
        # Resize x to a fixed size (e.g., using bilinear interpolation)
        x_resized = F.interpolate(
            x, size=self.output_size, mode="bilinear", align_corners=False
        )
        # Apply fixed-size pooling
        x_pooled = self.pool(x_resized)
        return x_pooled
...
class UperNetDecoder(nn.Module):
    def __init__(
        ...
        # PPM Module
        self.ppm = []
        fc_dim = ppm_dim // len(pool_scales)
        for scale in pool_scales:
            self.ppm.append(
                nn.Sequential(
                    self.pool(scale),
                    nn.Conv2d(ppm_dim, fc_dim, kernel_size=1, bias=False),
                    nn.BatchNorm2d(fc_dim),
                    nn.ReLU(inplace=True),
                )
            )
        self.ppm = nn.ModuleList(self.ppm)
        self.ppm_last_conv = conv3x3_bn_relu(ppm_out_dim, inter_dim, 1)
        ...
        self.conv_fusion = conv3x3_bn_relu(
            len(fpn_inplanes) * inter_dim, inter_dim, 1
        )
        ...
        self.head = nn.Sequential(
            conv3x3_bn_relu(inter_dim, inter_dim, 1),
            nn.Conv2d(inter_dim, num_classes, kernel_size=1, bias=True),
        )
