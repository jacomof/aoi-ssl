...
from .faster_vit_seg import FasterVitSeg
...
from segmentation.models.faster_vit.faster_vit_any_res import faster_vit_0_any_res
...
class LitFasterViTSegmentation(PLBaseModel, SegmentationLoggerMixin):
    def __init__(
        self,
        parameters: dict,
        classes: list,
        encoder_ckpt_path: str = None,
        encoder_ckpt_model_params: dict = None,
        encoder_class: "LitiBot | LitMAE" = None,
    ):
        # Lazy import to avoid circular dependency
        if encoder_class is None:
            from pretrain.mae.lit_mae import LitMAE
            encoder_class = LitMAE


        if encoder_ckpt_path is not None:
            print(f"Loading encoder from checkpoint: {encoder_ckpt_path}")
            encoder_pl = encoder_class.load_from_checkpoint(
                encoder_ckpt_path,
                parameters=encoder_ckpt_model_params,
                model=MaskedAutoencoderFasterVit,
                strict=False
            )
            encoder = encoder_pl.get_encoder()
        else:
            encoder = faster_vit_0_any_res()
        parameters["encoder"] = encoder
        super().__init__(model=FasterVitSeg, parameters=parameters, classes=classes)
    ...
    def _initialize_optimizer_normal(self, decayed_lr: bool = True):
        ...
        """
        if decayed_lr:
            # Collect all blocks from all levels, shallowest to deepest
            all_blocks = []
            for level in self.model.encoder.levels:
                for block in level.blocks:
                    all_blocks.append(block)

            num_blocks = len(all_blocks)
            print(f"Total number of blocks in encoder: {num_blocks}")

            # Assign decayed learning rates
            param_groups = []
            ...
            # Patch embedding layer (usually gets the most decayed LR)
            param_groups.append({
                "params": self.model.encoder.patch_embed.parameters(),
                "lr": self.lr * (self.layer_decay ** num_blocks)
            })

            # Decoder (usually trained from scratch, so base LR)
            param_groups.append({
