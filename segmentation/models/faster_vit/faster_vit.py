    ...
    def forward(self, input_tensor, local_window_size):
        ...
        if not self.grid_exists:
            ...
            relative_position_bias = torch.nn.functional.pad(
                ...
                (n_global_feature, 0, n_global_feature, 0),
            ).contiguous()
            if n_global_feature > 0 and self.ct_correct:
...
class ConvSegHead(nn.Module):
    def __init__(self, in_channels, inner_channels, num_classes, scale_factor=2):
        ...
        self.upsample2 = nn.Upsample(size=(128, 128))
        self.upsample3 = nn.Upsample(size=(256, 256))
...
class SegFasterViT(nn.Module):
    """FasterViT with Segmentation upsampling head"""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.head = UperNetDecoder(
            ppm_dim=ppm_dim,
            ppm_out_dim=ppm_out_dim,
