...
def faster_vit_0_ssl_any_res(
    pretrained=False, in_chans=2, input_resolution=224, **kwargs
):
    depths = kwargs.pop("depths", [2, 3, 6, 5])
    num_heads = kwargs.pop("num_heads", [2, 4, 8, 16])
    window_size = kwargs.pop("window_size", [7, 7, 7, 7])
...
class FasterViTSSL(FasterViT):
    ...
    def forward(
        ...
        patch_tokens = x.permute(0, 2, 3, 1).flatten(
            1, 2
        )  # [B, C, H, W] -> [B, H*W, C]
