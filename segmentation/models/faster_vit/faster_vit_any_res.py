...
class FasterViT(nn.Module):
    ...
    def __init__(self,
        ...
        if layer_norm_last:
            print("Using LayerNorm at the end of the model")

        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.head = nn.Linear(num_features, num_classes) if num_classes > 0 else nn.Identity()
        self.apply(self._init_weights)

        self.mask_token = nn.Parameter(torch.zeros(1, dim, 1, 1)) # Try to use the same mask token for encoder and decoder
        ...
        final_H = H // (4 * (2 ** num_downsamples))
        final_W = W // (4 * (2 ** num_downsamples))

        self.num_patches = final_H * final_W
        print(f"Number of patches after downsampling: {self.num_patches}")

        self.embed_dim = dim * (2 ** num_downsamples)
        self.dim = dim
        print(f"Final feature map size: {final_H}x{final_W}, embedding dimension: {self.embed_dim}")
    ...
    def forward_intermediate(self, x, masks=None):
        """
        Forward pass for intermediate features.
        Args:
            x: input tensor.
            masks: optional masks for the input tensor.
        Returns:
            x: processed tensor after passing through the forward features.
        """
    ...
    def forward_features(self, x, masks=None):
        ...
        if masks is not None:
            dH = dW = int(np.sqrt(self.num_patches))
            masks = masks.view(B, dH, dW)
            if x.shape[2:4] != masks.shape[1:3]:
                htimes, wtimes = np.array(x.shape[2:4]) // np.array(masks.shape[1:3])
                masks = masks.repeat_interleave(htimes, 1).repeat_interleave(wtimes, 2)
            masks = masks.unsqueeze(1)
            mask_expanded = masks.expand_as(x)  # [B, C, H, W]
            # print(f"Input x shape: {x.shape}, Mask shape: {masks.shape}, Expanded mask shape: {mask_expanded.shape}")
            # print(f"Mask token shape: {self.mask_token.shape}")
            x = torch.where(mask_expanded, self.mask_token.to(x.dtype), x)
...
def faster_vit_0_any_res(pretrained=False, in_chans=2, input_resolution=224, **kwargs):
    ...
    if in_chans is not None:
        kwargs['in_chans'] = in_chans # kwargs seems to already have and in_chans key
...
def faster_vit_0_ssl_any_res(pretrained=False, in_chans=2, input_resolution=224, **kwargs):
    depths = kwargs.pop("depths", [2, 3, 6, 5])
    num_heads = kwargs.pop("num_heads", [2, 4, 8, 16])
    window_size = kwargs.pop("window_size", [7, 7, 7, 7])
    ct_size = kwargs.pop("ct_size", 2)
    ...
    if in_chans is not None:
        kwargs['in_chans'] = in_chans # kwargs seems to already have and in_chans key
    model = FasterViTSSL(depths=depths,
                      num_heads=num_heads,
                      window_size=window_size,
                      ct_size=ct_size,
                      dim=dim,
...
def faster_vit_1_any_res(pretrained=False, in_chans=2, input_resolution=224, **kwargs):
...
def faster_vit_1_ssl_any_res(pretrained=False, in_chans=2, input_resolution=224, **kwargs):
    depths = kwargs.pop("depths", [1, 3, 8, 5])
    num_heads = kwargs.pop("num_heads", [2, 4, 8, 16])
    window_size = kwargs.pop("window_size", [7, 7, 7, 7])
    ct_size = kwargs.pop("ct_size", 2)
...
class FasterViTSSL(FasterViT):
    """ Swin Transformer V2 for SSL tasks.
    ...
    def forward(self, x: torch.Tensor, masks: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        Args:
            ...
            masks: Optional masks for input images.
        Returns:
            Output tensor after passing through the model.
        """
        x = self.forward_features(x, masks) # B, C, H, W

        # x_region = self.norm(x)  # B L C
        # x = self.avgpool(x_region.transpose(1, 2))  # B C 1
        patch_tokens = x.permute(0, 2, 3, 1).flatten(1,2) # [B, C, H, W] -> [B, H*W, C]
        # print("Patch tokens shape:", patch_tokens.shape)
        cls_token = patch_tokens.mean(dim=1)  # [B, C]
