...
class LinearPatchClassifier():
    ...
    def __init__(self, 
        ...
        self.channels = self.embedding_dimension * self.depth  # Total channels after concatenation
        ...
        print(f"Linear classifier initialized with {self.embedding_dimension} embedding dimension and {self.num_classes} classes.")
    ...
    def extract_and_upsample_features(self, x, config):
        ...
        for feature in features:
            ...
            assert patches_per_side * patches_per_side == num_patches, f"num_patches {num_patches} is not a perfect square"
            
            # Reshape to spatial format: (batch, embed_dim, height, width)
            spatial_feature = feature.permute(0, 2, 1)  # (batch, embed_dim, num_patches)
        ...
        spatial_features = torch.concatenate(spatial_features, dim=1)  # Concatenate along the channel dimension
    ...
    def create_memory(self, all_loader: torch.utils.data.DataLoader, config):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(tqdm(all_loader, desc='Memory Creation loop')):
                ...
                y = data["class_mask"].detach()  # Shape: (bs, num_patches, pixels_per_patch, num_classes)
                ignore_mask = data["ignore_mask"].detach()  # Shape: (bs, num_patches, pixels_per_patch, num_classes)

                features = self.extract_and_upsample_features(x, config)  # List of features for each layer
            ...
            print(f"Patch feature size: " + str(feature_memory[0].shape) if feature_memory else "No features processed")
            print(f"Label size: " + str(label_memory[0].shape) if label_memory else "No labels processed")
            feature_memory = torch.cat(feature_memory, dim=0)  # Shape: (b, num_patches, d_k)
            label_memory = torch.cat(label_memory, dim=0)  # Shape: (b, num_patches, num_classes)
            ignore_mask_memory = torch.cat(ignore_mask_memory, dim=0)  # Shape: (b, num_patches, num_classes)
        ...
        feature_memory_flattened_train = feature_memory[tensor_train_indices] # b*num_patches, d_k
        label_memory_flattened_train = label_memory[tensor_train_indices] # b*num_patches, pixels_per_patch, num_classes
        ignore_mask_memory_flattened_train = ignore_mask_memory[tensor_train_indices] # b*num_patches, pixels_per_patch, num_classes

        feature_memory_flattened_val = feature_memory[tensor_val_indices] # b*num_patches, d_k
        label_memory_flattened_val = label_memory[tensor_val_indices] # b*num_patches, pixels_per_patch, num_classes
        ignore_mask_memory_flattened_val = ignore_mask_memory[tensor_val_indices] # b*num_patches, pixels_per_patch, num_classes

        tensor_dataset_train = TensorDataset(feature_memory_flattened_train, label_memory_flattened_train, ignore_mask_memory_flattened_train)
        tensor_dataset_val = TensorDataset(feature_memory_flattened_val, label_memory_flattened_val, ignore_mask_memory_flattened_val)
    ...
    def patchify_gt(self, gt, patch_size):
        ...
        gt = gt.flatten(1, 2)  # Flatten the patch dimensions -> bs, num_patches, patch_size*patch_size, c
        ...
        for epoch in range(self.num_epochs):
            for batch in tqdm(self.patch_dataloader_train, desc="Training"):
                ...
                if y_hat.shape[2:] != class_mask.shape[1:3]:
                    y_hat = F.interpolate(y_hat, size=class_mask.shape[1:3], mode="bilinear")
    ...
    def evaluate(self, dataloader=None):
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="Evaluating"):
                ...
                y_hat_binary = (y_hat_prob > self.threshold).long()  # Convert to binary [0, 1]
        ...
        for c in classwise_ious[0]:
            mean_ious[c] = np.mean([iou[c].item() for iou in classwise_ious if iou[c] != 0])
