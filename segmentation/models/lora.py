...
class LoRA(nn.Module):
    ...
    def __init__(
        ...
        self.alpha = alpha
        self.r = linear_a_q.out_features

    def forward(self, x) -> torch.Tensor:
        # Compute the original qkv
        qkv = self.qkv(x)  # Shape: (B, N, 3 * org_C)

        # Compute the new q and v components with alpha scaling
        scaling = self.alpha / self.r
        # Compute the new q and v components
        new_q = self.linear_b_q(self.linear_a_q(x)) * scaling
        new_v = self.linear_b_v(self.linear_a_v(x)) * scaling
